...
"""
arcgiservice = ArcGISservices()

feature_to_add = features.Feature(
    geometry={
        "type": "Point",
        "x": 4.868969,
        "y": 45.715150
...
##create_feature([feature_to_add], 'stations_test')
## remove_feature(['1789'], 'stations_test')
...
while True:
    if not added:
        ...
        remove_feature([identifier], 'stations_test')
    added = not added
    time.sleep(20)

while True:
    if not added:
        arcgiservice.update_feature([{
            "attributes": {
                "OBJECTID": 15,
                "capacity": 150
            }
...
params = {
    'pointLayer': {'url': arcgiservice.get_layer_with_name('stations_capacity').url},
    'polygonLayer': {'url': arcgiservice.get_layer_with_name('polygonLayer').url},
    'keepBoundariesWithNoPoints': False
}
arcgiservice.spatial_analysis(task='AggregatePoints', parameters=params)
"""
